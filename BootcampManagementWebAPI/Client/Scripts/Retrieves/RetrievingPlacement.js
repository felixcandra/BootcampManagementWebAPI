...
function LoadProvince(element) {
    if (Provinces.length == 0) {
        $.ajax({
            type: "GET", // get
            url: 'http://localhost:53126/api/Placement',
...
function Save() {
    ...
    item.Villages_Id = $('#Village').val();// masih ragu disini
    $.ajax({
        type: "POST", //insert
        url: "http://localhost:53126/api/Placement",
        datatype: "json",
...
function Edit() {
    ...
    $.ajax({
        ...
        url: "http://localhost:53126/api/Placement" + $('#Id').val(),
        datatype: "json",
        data: item,
...
function GetById(Id) {
    debugger;
    $.ajax({
        url: "http://localhost:53126/api/Placement" + Id, // search
        type: "GET",
        datatype: "json",
        success: function (item) {
            ...
            $('#Village').val(item.Villages.Id);//udah benar
            $('#myModal').modal('show');
            $('#Update').show();
            $('#Save').hide();
        }
...
function Delete(Id) {
    swal({ // popup konfirmasi delete swal 
        ...
        $.ajax({
            url: "http://localhost:53126/api/Placement" + Id,
            type: "DELETE",
