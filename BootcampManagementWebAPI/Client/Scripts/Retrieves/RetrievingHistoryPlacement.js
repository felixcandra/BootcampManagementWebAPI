...
function LoadProvince(element) {
    if (Provinces.length == 0) {
        $.ajax({
            type: "GET", // get
            url: 'http://localhost:53126/api/HistoryPlacement',
...
function Save() {
    ...
    item.Placemens_Id = $('#Placement').val();
...
function Edit() {
    ...
    item.Placement = $('#Placement').val();
    item.Students_Id = $('#Student').val();// masih ragu disini
    $.ajax({
        type: "PUT", //put untuk update
        url: "http://localhost:53126/api/HistoryPlacement" + $('#Id').val(),
        datatype: "json",
        data: item,
...
function GetById(Id) {
    debugger;
    $.ajax({
        url: "http://localhost:53126/api/HistoryPlacement" + Id, // search
        type: "GET",
        datatype: "json",
        success: function (item) {
            ...
            $('#Placement').val(item.Placement.Id)
...
function Delete(Id) {
    swal({ // popup konfirmasi delete swal 
        ...
        $.ajax({
            url: "http://localhost:53126/api/HistoryPlacement" + Id,
            type: "DELETE",
...
function validationInsert() {
    ...
    if ($('#StartDate').val() == "" || ($('#StartDate').val() == " ")) {
        isAllValid = false; //kalau textbox nama kosong maka
        $('#StartDate').siblings('span.error').css('visibility', 'visible'); //ini notifikasi buat ngasi tau field belum diisi pas mencet save 
    }
    if ($('#EndDate').val() == "" || ($('#EndDate').val() == " ")) {
        isAllValid = false; //kalau textbox nama kosong maka
        $('#EndDate').siblings('span.error').css('visibility', 'visible'); //ini notifikasi buat ngasi tau field belum diisi pas mencet save 
    }
